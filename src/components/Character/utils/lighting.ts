...
const setLighting = (scene: THREE.Scene) => {
  ...
  new RGBELoader()
    .setPath(`${assetBase}models/`)
    .load("char_enviorment.hdr?v=2", (texture: THREE.Texture) => {
  ...
  function setPointLight(screenLight: THREE.Object3D | null) {
    const asMesh = screenLight as THREE.Mesh | null;
    const material = asMesh?.material as THREE.MeshStandardMaterial | undefined;
